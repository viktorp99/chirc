...
public abstract class Stream<A> implements Supplier<Stream<String>> {
    ...
    public Stream<A> takeViaUnfold(int n) {
        return unfold(new Tuple<>(this, n),
                x -> x._1.isEmpty() ? Result.empty()
                        : x._2 > 0 ? Result.success(new Tuple<>(x._1.head()._1, new Tuple<>(x._1.tail(), x._2 - 1)))
                        : Result.empty());
    }
    ...
    public <B> Stream<B> mapViaUnfold(Function<A, B> f) {
        return unfold(this,
                x -> x.isEmpty() ? Result.empty() : Result.success(new Tuple<>(f.apply(x.head()._1), x.tail())));
    }
    ...
    /**
     * Not optimized because x._1.head() is called twice and x._1.tail() is fine
     * because the tail does not change after calling head().
     */
    public Stream<A> takeWhileViaUnfold(Function<A, Boolean> f) {
        return unfold(new Tuple<>(this, f),
                x -> x._1.isEmpty() ? Result.empty()
    ...
    private TailCall<Boolean> exists(Stream<A> s, Function<A, Boolean> p) {
        return s.isEmpty() ? ret(false) : p.apply(s.head()._1) ? ret(true) : sus(() -> exists(s.tail(), p));
    }
    ...
    private TailCall<Stream<A>> dropWhile(Stream<A> acc, Function<A, Boolean> f) {
        return acc.isEmpty() ? ret(acc) : f.apply(acc.head()._1) ? sus(() -> dropWhile(acc.tail(), f)) : ret(acc);
    }
    ...
    private TailCall<List<A>> toList(Stream<A> s, List<A> acc) {
        return s.isEmpty() ? ret(acc) : sus(() -> toList(s.tail(), List.cons(s.head()._1, acc)));
    }
    ...
    public <B, C> Stream<C> zipAllGeneric(Stream<B> that, Function<Result<A>, Function<Result<B>, C>> f) {
        ...
        return unfold(new Tuple<>(a, b),
                x -> x._1.isEmpty() && x._2.isEmpty() ? Result.empty()
                        : x._1.head()._1.isEmpty() && x._2.head()._1.isEmpty() ? Result.empty()
                        : Result.success(new Tuple<>(f.apply(x._1.head()._1).apply(x._2.head()._1),
                        new Tuple<>(x._1.tail(), x._2.tail()))));
    }
    ...
    public Stream<Stream<A>> tails() {
        return cons(() -> this, () -> unfold(this, x -> x.isEmpty() ? Result.empty() // Result.<Tuple<Stream<A>,Stream<A>>>
    ...
    private static class Empty<A> extends Stream<A> {
        ...
        /**
         * foreach method for the subclass  nil
    ...
    private static class Cons<A> extends Stream<A> {
        ...
        public Tuple<A, Stream<A>> head() {
            ...
            return h.isEmpty() ? new Tuple<>(a, new Cons<>(a, tail)) : new Tuple<>(a, this);
        }
        ...
        @Override
        public Stream<A> takeWhile_(Function<A, Boolean> f) {
            return f.apply(head()._1) ? cons(head, () -> tail().takeWhile_(f)) : empty();
        }
        ...
        public TailCall<Stream<A>> drop(Stream<A> acc, int n) {
            return acc.isEmpty() || n <= 0 ? ret(acc) : sus(() -> drop(acc.tail(), n - 1));
        }

        /**
         * foreach method that applies a given effect ef to each elemet in the stream
         */
        @Override
        public void forEach(Effect<A> ef) {
            ef.apply(head()._1);
    ...
    public static <A> Stream<A> stream(List<A> list) {
        return list.isEmpty() ? empty() : cons(list::head, () -> stream(list.tail()));
    }
    ...
    public static Stream<Integer> range(int start, int end) {
        return start > end ? Stream.empty() : cons(() -> start, () -> range(start + 1, end));
    }
    ...
    public static <T> TailCall<Stream<T>> fill(Stream<T> acc, int n, Supplier<T> elem) {
        return n <= 0 ? ret(acc) : sus(() -> fill(new Cons<T>(elem, () -> acc), n - 1, elem));
    }
